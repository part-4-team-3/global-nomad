...
export const InputStyles = {
  base: 'px-20pxr py-16pxr border border-var-gray2 rounded-md focus:outline-var-green-dark',
...
const Input = forwardRef<HTMLInputElement, Props>(
  ({ size, hasError = false, className = '', ...rest }, ref) => {
    return (
      <input
        ref={ref}
        className={`${InputStyles.base} ${InputStyles[size]} ${hasError && InputStyles.error} ${className}`}
        {...rest}
      />
    );
  },
